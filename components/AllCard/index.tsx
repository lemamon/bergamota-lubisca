...
import { EVENT_TITLE, EVENT_SUBTITLE, EVENT_INTRO, MENU_CONTENT, EVENT_CLOSING, EVENT_CTA } from '../../constants';
          ...
          <div className="w-full space-y-20">
            {MENU_CONTENT.map((section) => (
              <div key={section.title} className="flex flex-col items-center">
                    ...
                    <div key={idx} className="flex flex-col items-center">
                      ...
                      <div className="mb-4">
                        {item.brand === 'bergamota' ? (
                           <BergamotIcon className="w-12 h-12 text-brand-orange" />
                        ) : item.brand === 'lubisca' ? (
                           <SmallJarIcon className="w-10 h-10 text-brand-pink" />
                        ) : (
                      ...
                      {item.ingredients && (
                        <ul className="flex flex-col gap-2 items-center">
                          {item.ingredients.map((ing, i) => (
                            <li key={i} className="font-sans font-light text-brand-dark/70 uppercase tracking-widest text-sm md:text-base">
                              {ing}
                            </li>
                          ))}
                        </ul>
                ...
                <div className="mt-16 w-full flex justify-center opacity-30 text-brand-purple">
                  <span>♦</span>
